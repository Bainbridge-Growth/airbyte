...
### Issue: ResultSetBigError (10100) with First Dimension
**Symptom:** QuickBooks returns error 10100 "Result Set Big Error" when using `first_dimension` with many unique values (e.g., 20+ classes/departments)
**Cause:** Report becomes too wide with a column for each dimension item
**Solution:** The connector automatically falls back to fetching each dimension item separately (similar to `second_dimension` behavior). This is handled by:
- `ResultSetBigError` exception class
- `_first_dimension_fallback_mode` flag in `QuickbooksReportMonthlyBase`
- `_read_records_with_first_dimension_fallback()` method
- The fallback makes individual API calls with dimension filters instead of using `summarize_column_by`
...
- Fetches dimension items (classes/departments/etc.)
- Makes individual API calls filtered by each dimension
- Sets `Class` field to dimension name on each record
